...
int main(int argc, char *argv[]) {
    ...
    // Allocate x and local y
    x = (float *) malloc(M * sizeof(float));
    y_local = (float *) malloc((end_row - start_row) * sizeof(float));

    // Root initializes x and broadcasts
    if (rank == 0) {
        for (i = 0; i < M; i++)
            x[i] = (M / 2.0 - i);
    }

    MEC(MPI_Ibcast(x, M, MPI_FLOAT, 0, MPI_COMM_WORLD, &reqs[0]));
    MEC(MPI_Wait(&reqs[0], MPI_STATUS_IGNORE));

    MEC(MPI_Barrier(MPI_COMM_WORLD));
    t_start = MPI_Wtime();

    // Matrix-vector local product with OpenMP
    ...
    MEC(MPI_Igather(y_local, end_row - start_row, MPI_FLOAT,
                    y_global, end_row - start_row, MPI_FLOAT,
                    0, MPI_COMM_WORLD, &reqs[1]));
    MEC(MPI_Wait(&reqs[1], MPI_STATUS_IGNORE));

    MEC(MPI_Barrier(MPI_COMM_WORLD));
    t_end = MPI_Wtime();

    // Debug output - per-process accumulation
