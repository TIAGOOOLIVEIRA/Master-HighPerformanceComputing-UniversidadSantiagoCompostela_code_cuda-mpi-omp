...
void smvp(int nodes, double ***A, int *Acol, 
  ...
  //double sumw0, sumw1, sumw2;
    ...
    for (i = 0; i < nodes; i++) {
      ...
      //sumw0 = w[Acol[Anext + 1]][0], sumw1 = w[Acol[Anext + 1]][1], sumw2 = w[Acol[Anext + 1]][2];

    //#pragma omp parallel firstprivate ( Anext ) reduction ( + : sum0, sum1, sum2, sumw0, sumw1, sumw2)
      ...
      while (Anext < Alast) {
       ...
       /*
        sumw0 += w[col][0] + A[Anext][0][0]*v[i][0] + A[Anext][1][0]*v[i][1] + A[Anext][2][0]*v[i][2];
        sumw1 += w[col][1] + A[Anext][0][1]*v[i][0] + A[Anext][1][1]*v[i][1] + A[Anext][2][1]*v[i][2];
        sumw2 += w[col][2] + A[Anext][0][2]*v[i][0] + A[Anext][1][2]*v[i][1] + A[Anext][2][2]*v[i][2];
      */
        Anext++;
      }
